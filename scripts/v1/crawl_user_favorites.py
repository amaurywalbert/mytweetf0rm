...
if __name__=="__main__":
	...
	parser.add_argument('-s', '--seeds', help="the list of users you want to crawl their favorites; see crawl_user_favorites.json as an example", required = True)
	parser.add_argument('-o', '--output', help="define the location of the output (each user's favorites will be in its own file under this output folder identified by the user id", required = True)
	...
	with open(os.path.abspath(args.apikeys), 'rb') as apikeys_f, open(os.path.abspath(args.seeds), 'rb') as config_f:
		...
		while True:				
			try:
				farm_user_favorites(apikeys, seeds, args.output)
			except KeyboardInterrupt:
				logger.warn('You pressed Ctrl+C!')
				sys.exit(0)
			except:
