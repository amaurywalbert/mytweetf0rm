...
##########################################################################################################################################################################
	...
	def fetch_user_retweets(self, user_id = None, write_to_handlers=[], cmd_handlers=[], bucket="retweets"):

		if not user_id:
			raise Exception("user_retweets: user_id cannot be None")
		...
		retry_cnt = MAX_RETRY_CNT
		retweets = [] # holder tweets in memory... you won't get more than 3,200 tweets per user, so I guess this is fine...
		while current_max_id != prev_max_id and retry_cnt > 1:
			try:
				if current_max_id > 0:
					tweets = self.get_user_timeline(user_id=user_id, max_id=current_max_id - 1, count=200)
				...
				self.rate_limit_error_occured('statuses', '/statuses/user_retweets')
			except Exception as exc:
				time.sleep(10)
				logger.debug("exception: %s"%exc)
				retry_cnt -= 1
		...
		if (len(retweets) > 0):
			for tweet in retweets:
				if hasattr(tweet, 'retweeted_status'):
					status = {'retweet_id':tweet['retweeted_status']['id'],'author_id': tweet['retweeted_status']['user']['id']}

					for handler in write_to_handlers:
						handler.append(json.dumps(status), bucket=bucket, key=user_id)
		...
		logger.debug("[%s] total retweets: %d "%(user_id, cnt))	

##########################################################################################################################################################################
##########################################################################################################################################################################

	def fetch_user_favorites(self, user_id = None, write_to_handlers=[], cmd_handlers=[], bucket="favorites"):

		if not user_id:
			raise Exception("user_favorites: user_id cannot be None")
		...
		retry_cnt = MAX_RETRY_CNT
		favorites = [] # holder tweets in memory... you won't get more than 3,200 tweets per user, so I guess this is fine...
		while current_max_id != prev_max_id and retry_cnt > 1:
			try:
				if current_max_id > 0:
					tweets = self.get_favorites(user_id=user_id, max_id=current_max_id - 1, count=200)
				else:
					tweets = self.get_favorites(user_id=user_id, count=200)
				...
				self.rate_limit_error_occured('statuses', '/statuses/user_favorites')
			except Exception as exc:
				time.sleep(10)
				logger.debug("exception: %s"%exc)
				retry_cnt -= 1
		...
		if (len(favorites) > 0):
			for tweet in favorites:
				...
				status = {'favorite_id':tweet['id'],'author_id': tweet['user']['id']}
				for handler in write_to_handlers:
					handler.append(json.dumps(status), bucket=bucket, key=user_id)
...
##########################################################################################################################################################################
	...
	def fetch_user_mentions(self, user_id = None, write_to_handlers=[], cmd_handlers=[], bucket="mentions"):

		if not user_id:
			raise Exception("user_mentions: user_id cannot be None")
		...
		retry_cnt = MAX_RETRY_CNT
		mentions = [] # holder tweets in memory... you won't get more than 3,200 tweets per user, so I guess this is fine...
		while current_max_id != prev_max_id and retry_cnt > 1:
			try:
				if current_max_id > 0:
					tweets = self.get_user_timeline(user_id=user_id, max_id=current_max_id - 1, count=200)
				...
				self.rate_limit_error_occured('statuses', '/statuses/user_mentions')
			except Exception as exc:
				time.sleep(10)
				logger.debug("exception: %s"%exc)
				retry_cnt -= 1
		...
		if (len(mentions) > 0):
			for tweet in mentions:
				if not hasattr(tweet, 'retweeted_status'):	#Despreza retweets..
					mentioneds = tweet['entities']['user_mentions']
					status = {'tweet_id':tweet['id'],'mentioneds': mentioneds}
		...
		logger.debug("[%s] total mentions: %d "%(user_id, cnt))	
