...
class TwitterCrawler(CrawlerProcess):
	...
	def __init__(self, node_id, crawler_id, apikeys, handlers, redis_config, proxies=None):
		...
		self.tasks = {
			...
			"CRAWL_USER_FAVORITES": "fetch_user_favorites",
			"CRAWL_USER_RETWEETS": "fetch_user_retweets",
			"CRAWL_USER_MENTIONS": "fetch_user_mentions",
	...
	def run(self):
		while True:
			...
			if (command == 'TERMINATE'):
				...
				func = None
				if  (command in ['CRAWL_USER_TIMELINE', 'CRAWL_TWEET','CRAWL_USER_FAVORITES','CRAWL_USER_RETWEETS','CRAWL_USER_MENTIONS']):
